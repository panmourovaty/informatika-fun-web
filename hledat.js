...
function searchData() {
    ...
    if (filteredResults.length > 0) {
        filteredResults.forEach(item => {
            resultsDiv.innerHTML += `<div class="col-4 p-3 h-100"><a href="${item.url}"><div class="bg-frostedglass p-4 text-center"><b>${item.name}</b><img src="${item.img}" height="80" class="p-2"/><p>${item.desc}</p></div></a></div>`; // Change 'name' to the appropriate key
        });
    } else {
